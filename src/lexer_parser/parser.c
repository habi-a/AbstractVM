...
t_ast_node          *expression(t_parse_utils *parse_utils, t_stack_node *stack)
{
    ...
    term_ast_node   = term(parse_utils, stack);
...
t_ast_node          *term(t_parse_utils *parse_utils, t_stack_node *stack)
{
    ...
    fact_ast_node   = factor(parse_utils, stack);
...
t_ast_node          *factor(t_parse_utils *parse_utils, t_stack_node *stack)
{
    ...
    switch (parse_utils->current_token.token_type)
    {
        case TOK_LEFT_PAREN:
            ...
            ast_node = expression(parse_utils, stack);
            expect(')', parse_utils);
            return (ast_node);
        case TOK_NUMBER:
        ...
        case TOK_INSTRUCTION:
            ...
            if (parse_utils->current_token.token_type == TOK_END_TEXT)
                ...
                ast_node1 = factor(parse_utils, stack);
        ...
        case TOK_FUNC_CALL:
            ...
            ast_node1 = expression(parse_utils, stack);
            expect(')', parse_utils);
...
t_ast_node            *parse(const char *line, t_stack_node *stack)
{
    ...
    pop_token(&parse_utils);
    return (expression(&parse_utils, stack));
}
